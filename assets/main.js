/* ── THUMBNAIL FALLBACK ─────────────────────────────────── */
// For YouTube Shorts that block maxresdefault, fall back to hqdefault
document.querySelectorAll('.ratio-v .project-bg').forEach(el => {
  ...
  const img = new Image();
  img.onload = () => {
    // maxresdefault loaded fine — if it's YouTube's 120x90 placeholder, try hqdefault
    if (img.naturalWidth <= 120) tryFallback(el, id);
  };
...
function tryFallback(el, id) {
  ...
  img2.onerror = () => {
    el.style.backgroundImage = 'linear-gradient(135deg, #111 0%, #1a1a1a 100%)';
  };
